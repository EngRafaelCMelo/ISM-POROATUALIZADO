...
def test_invalid_values_do_not_update_maxima(tmp_path) -> None:
    ...
    repo.save_measurement(
        ...
        Measurement(
            datetime.now(),
            pressure=SensorReading(value=999, quality=ReadingQuality.INVALID),
            flow=SensorReading(value=999, quality=ReadingQuality.INVALID),
        ),
    )
...
def test_pause_is_excluded_from_duration(tmp_path) -> None:
    ...
    db.initialize()
    service = Service(Repository(db), EventRepository(db))
    session = service.start(Definition("ENS-2", "A"))
    session.started_at = datetime.now()
    service.toggle_pause()
    service._paused_at = datetime.now()
...
def test_wal_backup_and_integrity(tmp_path) -> None:
    ...
    db = Database(manager.database_path)
    db.initialize()
...
def test_old_database_migrates_without_losing_flow(tmp_path) -> None:
    ...
    with db.transaction() as con:
        con.execute(
            "INSERT INTO ensaios(codigo,amostra_nome,operador,inicio,status) VALUES('OLD','A','O','2026-01-01','finalizado')"
        )
        con.execute(
            "INSERT INTO medicoes(ensaio_id,timestamp_computador,vazao_baixa,qualidade) VALUES(1,'2026-01-01',1.25,'válida')"
        )
    db.initialize()
