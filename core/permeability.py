...
GAS_PROPERTIES = {
    "Helio": {"nome": "Hélio", "viscosidade_upa_s": 19.60},
    "Nitrogenio": {"nome": "Nitrogênio", "viscosidade_upa_s": 17.57},
    "Ar": {"nome": "Ar", "viscosidade_upa_s": 18.13},
    "Argonio": {"nome": "Argônio", "viscosidade_upa_s": 22.23},
    "CO2": {"nome": "Dióxido de carbono", "viscosidade_upa_s": 14.68},
}
...
def calculate_gas_permeability(
    *,
    ...
    q, mu, length, diameter = (
        flow_l_min / 60000,
        viscosity_upa_s * 1e-6,
        length_mm / 1000,
        diameter_mm / 1000,
    )
    area = math.pi * diameter**2 / 4
    pin, pout, pref = (
        inlet_pressure_kpa_abs * 1000,
        outlet_pressure_kpa_abs * 1000,
        reference * 1000,
    )
    ...
    return PermeabilityResult(
        ...
        k / DARCY_M2,
        k / DARCY_M2 * 1000,
        inlet_pressure_kpa_abs - outlet_pressure_kpa_abs,
        (inlet_pressure_kpa_abs + outlet_pressure_kpa_abs) / 2,
        q / area,
        (pin - pout) / length,
    )
...
def calculate_klinkenberg(points: Iterable[tuple[float, float]]) -> KlinkenbergResult:
    ...
    d = sum((x - xm) ** 2 for x in xs)
    if d == 0:
        raise ValueError("Use pressões médias distintas")
    slope = sum((x - xm) * (y - ym) for x, y in zip(xs, ys)) / d
