...
class ExportService:
    ...
    def _current_measurements(measurements: pd.DataFrame) -> pd.DataFrame:
        """Coalesce schema atual/legado sem criar nomes duplicados."""
    ...
    def _records(frame: pd.DataFrame) -> list[dict[str, Any]]:
        """Converte valores ausentes do pandas em null JSON válido."""
    ...
    def export_json(self, test_id: int, directory: Path) -> Path:
        ...
        payload = {
            "ensaio": dict(test),
            "medicoes": self._records(measurements),
    ...
    def export_pdf(self, test_id: int, directory: Path) -> Path:
        ...
        details = [
            ["Versão do aplicativo", APP_VERSION, "Firmware", test["versao_firmware"] or "—"],
            ["Código", test["codigo"], "Status", test["status"]],
            ["Amostra", test["amostra_nome"], "Operador", test["operador"]],
            ["Início", test["inicio"], "Fim", test["fim"] or "—"],
            [
                "Amostras",
                str(test["quantidade_amostras"]),
                "Duração (s)",
                f"{test['duracao_segundos']:.1f}",
            ],
            ["Gás", test["tipo_gas"] or "—", "Temperatura", f"{test['temperatura_c'] or 0:.2f} °C"],
            [
                "L / D",
                f"{test['comprimento_amostra_mm'] or '—'} / {test['diametro_amostra_mm'] or '—'} mm",
                "Massa",
                f"{test['massa_amostra_g'] or '—'} g",
            ],
        ]
        ...
        story.extend(
            [table, Spacer(1, 7 * mm), Paragraph("Resumo estatístico", styles["Heading2"])]
        )
        stat_rows = [["Variável", "Mínimo", "Média", "Máximo", "Desvio padrão"]]
        measurements = self._current_measurements(measurements)
        for column, label in [("pressao", "Pressão"), ("vazao", "Vazão")]:
        ...
        story.append(Paragraph("Rastreabilidade das aquisições", styles["Heading2"]))
        trace_rows = [
            ["Sensor", "Unidade", "Válidas / total", "Primeira leitura", "Última leitura"]
        ]
        total = len(measurements)
        for label, unit, valid_column, timestamp_column in (
            ("Pressão", test["unidade_pressao"] or "bar", "pressao_valida", "timestamp_pressao"),
            ("Vazão", test["unidade_vazao"] or "NL/min", "vazao_valida", "timestamp_vazao"),
        ):
            valid = (
                int(pd.to_numeric(measurements[valid_column], errors="coerce").fillna(0).sum())
                if valid_column in measurements
                else 0
            )
            timestamps = (
                measurements[timestamp_column].dropna().astype(str)
                if timestamp_column in measurements
            ...
            trace_rows.append(
                [
                    ...
                    unit,
                    f"{valid} / {total}",
                    timestamps.iloc[0] if not timestamps.empty else "—",
                    timestamps.iloc[-1] if not timestamps.empty else "—",
                ]
            )
        trace_table = Table(
            trace_rows,
            repeatRows=1,
            colWidths=[22 * mm, 19 * mm, 27 * mm, 49 * mm, 49 * mm],
        )
        ...
        story.extend([trace_table, Spacer(1, 7 * mm)])
        if not calculations.empty:
            ...
            for _, calculation in calculations.iterrows():
                ...
                if calculation["tipo"] == "Lei de Boyle":  # registro legado somente leitura
                    ...
                    principal = (
                        f"k∞={self._format_result(result.get('intrinsic_permeability_md'), 'mD')}"
                    )
